...
var isServer = componentsUtil.aF_ === true;
...
function resolveComponentKey(key, parentComponentDef) {
  if (key[0] === "#") {
    ...
    return parentComponentDef.id + "-" + parentComponentDef._J_(key);
  }
}
...
function handleBeginAsync(event) {
  ...
  asyncOut.c(parentOut.m_, parentOut.o_, parentOut.aE_);
}
  ...
  return function renderer(input, out) {
    ...
    if (isServer) {
      ...
      if (!component) {
        ...
        if (isExisting === true) {
          if (component._m_ === false || component.shouldUpdate(input, component.N_) === false) {
            // We put a placeholder element in the output stream to ensure that the existing
            // DOM node is matched up correctly when using morphdom. We flag the VElement
            // node to track that it is a preserve marker
            out.aH_(component);
            globalComponentsContext._T_[id] = true;
            component.M_(); // The component is no longer dirty so reset internal flags
            return;
          }
        }
      }

      component.W_ = out.global;
...
createRendererFunc.as_ = resolveComponentKey;
createRendererFunc.aD_ = trackAsyncComponents;