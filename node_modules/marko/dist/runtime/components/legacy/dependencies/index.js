...
function getRootDeps(template, context) {
  if (production && template.aA_) {
    return template.aA_;
  }

  attachDepsAndComponentsToTemplate(template, context);

  var deps = template.aA_ = Object.keys(template.aB_).map(key => template.aB_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
  ...
  if (production && target.aB_) return;
  ...
  if (meta.tags) {
    meta.tags.forEach(tagPath => {
      ...
      if (tag.aB_) Object.assign(deps, tag.aB_);
