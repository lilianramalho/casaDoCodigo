...
module.exports = function dynamicTag(out, tag, getAttrs, renderBody, args, props, componentDef, key, customEvents) {
  if (tag) {
    ...
    var component = componentDef && componentDef.n_;
    if (typeof tag === "string") {
      ...
      if (renderBody) {
        out.aT_(tag, attrs, key, componentDef, addEvents(componentDef, customEvents, props));
        renderBody(out);
        out.aU_();
      } else {
        out.aV_(tag, attrs, key, componentDef, addEvents(componentDef, customEvents, props));
      }
    } else {
      if (attrs == null) {
      ...
      if (renderer) {
        ...
        if (isFn) {
          ...
          out.q_(key, component, preserve);
          if (!isW10NOOP && isFn) {
            var componentsContext = getComponentsContext(out);
            var parentComponentDef = componentsContext._O_;
            var globalContext = componentsContext.k_;
            componentsContext._O_ = new ComponentDef(component, parentComponentDef.id + "-" + parentComponentDef._J_(key), globalContext);
...
function attrsToCamelCase(attrs) {
  ...
  for (var key in attrs) {
    result[changeCase.aS_(key)] = attrs[key];
  }
