...
AsyncValue.prototype = {
   ...
   * available.
   */
  y_: function (callback) {
    // Do we already have data or error?
    if (this.x_) {
      // invoke the callback immediately
      return callback(this.v_, this.u_);
    }

    var callbacks = this.w_ || (this.w_ = []);
  ...
  A_: function (value) {
    ...
    if (value && typeof value.then === "function") {
      ...
      var finalPromise = value.then(function onFulfilled(value) {
        queueMicrotask(asyncValue.A_.bind(asyncValue, value));
      }, function onRejected(err) {
        queueMicrotask(asyncValue.z_.bind(asyncValue, err));
      });
